    ...
    <aside className="w-64 bg-white border-r border-slate-200/80 shrink-0 min-h-[calc(100vh-69px)] p-4 shadow-sm">
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <Icon className={`w-4 h-4 ${isActive ? 'text-vamorange-500' : 'text-slate-400'}`} />
                <span>{link.label}</span>
              </div>
              {isActive && <ChevronRight className="w-3.5 h-3.5 text-vamorange-500" />}
            </Link>
          );
        })}
      </nav>
      ...
      {role === 'admin' && (
        <div className="mt-8 pt-4 border-t border-slate-200">
          ...
          <Link
            href="/dashboard"
            className="flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl text-xs font-semibold text-slate-700 hover:bg-slate-100 transition-all"
          >
